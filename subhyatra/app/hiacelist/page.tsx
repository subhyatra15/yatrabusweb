...
const getAmenities = (hiace: any) => {
  const amenities = [];
  if (hiace.wifi) amenities.push("wifi");
  if (hiace.charging) amenities.push("charging");
  if (hiace.ac) amenities.push("ac");
  if (hiace.tv) amenities.push("tv");
  if (hiace.water) amenities.push("water");
...
function HiaceListPageComp() {
  ...
  const [selectedHiace, setSelectedHiace] = useState<any>(null);
  const [routeStops, setRouteStops] = useState<any[]>([]);
  const [boardingStops, setBoardingStops] = useState<any[]>([]);
  const [droppingStops, setDroppingStops] = useState<any[]>([]);
  const [selectedBoardingStop, setSelectedBoardingStop] = useState<any>(null);
  const [selectedDroppingStop, setSelectedDroppingStop] = useState<any>(null);
  const [isLoadingStops, setIsLoadingStops] = useState(false);
  const [isLoadingPrice, setIsLoadingPrice] = useState(false);
  const [pricePerSeat, setPricePerSeat] = useState<number | null>(null);
  const [hiaces, setHiaces] = useState<any[]>([]);
  const [filteredHiaces, setFilteredHiaces] = useState<any[]>([]);
  ...
  // Fetch Hiaces from API
  const fetchHiaces = async () => {
    try {
      setIsLoading(true);
      ...
      if (response.data) {
        const transformedHiaces = response.data?.results?.map((hiace: any) => ({
          id: hiace.id.toString(),
          scheduleId: hiace.id,
          routeId: hiace.route,
          name: hiace.hiace_name || "Hiace",
          type: hiace.hiace_type || (hiace.ac ? "AC" : "Non-AC"),
          from: hiace.source_city || from,
          ...
          rating: hiace.rating || 4.3,
          amenities: getAmenities(hiace),
          hiaceNumber: hiace.hiace_number || "N/A",
          hiaceId: hiace.hiace,
          status: hiace.status,
          seatLayout: hiace.seat_layout || { left: 2, right: 2 },
        }));

        setHiaces(transformedHiaces);
        setFilteredHiaces(transformedHiaces);
      } else {
        setHiaces([]);
        setFilteredHiaces([]);
      }
    } catch (error: any) {
      console.error("Error fetching hiaces:", error);
      if (error.response) {
  ...
  const fetchRouteStops = async (routeId: number) => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/api/v1/hiace-routes/routestop/?routeid=${routeId}`,
        {
          headers: {
            "Content-Type": "application/json",
            ...(token && { Authorization: `Bearer ${token}` }),
  ...
  const fetchPricePerSeat = async (routeId: number, boardingStopId: number, droppingStopId: number) => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/api/v1/hiace-routes/priceperseat/`,
        {
          headers: {
            "Content-Type": "application/json",
            ...(token && { Authorization: `Bearer ${token}` }),
  ...
  const navigateToHiaceDetails = (hiace: any, boardingStop: any, droppingStop: any, price?: number) => {
    ...
    router.push(
      `/hiacedetails?id=${hiace.id}&scheduleId=${hiace.scheduleId}&routeId=${hiace.routeId}&boardingStopId=${boardingStop?.id || ""}&droppingStopId=${droppingStop?.id || ""}&boardingCity=${boardingStop?.city_name || ""}&droppingCity=${droppingStop?.city_name || ""}&price=${finalPrice}`
    );
  };

  // Handle book now
  const handleBookNow = (hiace: any) => {
    setSelectedHiace(hiace);
    setPricePerSeat(null);
  ...
  useEffect(() => {
    ...
    if (selectedFilter !== "All") {
      result = result.filter((hiace) => hiace.type === selectedFilter);
    }
    ...
    setFilteredHiaces(result);
  }, [hiaces, selectedFilter, sortBy]);

  // Load hiaces on mount
  useEffect(() => {
    fetchHiaces();
  }, []);
  ...
  // Hiace Card Component
  const HiaceCard = ({ hiace }: { hiace: any }) => (
    <motion.div
      ...
      <div className="p-5">
        ...
        <div className="flex items-center gap-4 py-3 border-y border-slate-100 mb-3">
          <div className="flex flex-col items-center gap-1">
            ...
            <div className="w-3 h-3 rounded-full bg-emerald-400 border-2 border-emerald-200" />
          </div>
          <div className="flex-1 flex justify-between items-center">
            <div>
              <p className="font-bold text-gray-900">{hiace.departure}</p>
              <p className="text-sm text-slate-400">{hiace.from}</p>
            </div>
            <div className="flex-1 mx-4 text-center">
              <div className="h-px bg-slate-200 w-full" />
              <p className="text-xs text-slate-400 mt-1">{hiace.duration}</p>
              <div className="h-px bg-slate-200 w-full" />
            </div>
            <div className="text-right">
              <p className="font-bold text-gray-900">{hiace.arrival}</p>
              <p className="text-sm text-slate-400">{hiace.to}</p>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-4 pb-24">
        ...
        >
          ...
          <motion.button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-50 text-emerald-600 font-semibold text-sm whitespace-nowrap hover:bg-emerald-100 transition-colors"
          >
        ...
        <div className="mb-4">
          <p className="text-sm text-slate-400 font-medium">
            {filteredHiaces.length} hiaces found
          </p>
        </div>

        {/* Hiace List */}
        <AnimatePresence mode="wait">
          {filteredHiaces.length > 0 ? (
            <div className="space-y-4">
              {filteredHiaces.map((hiace) => (
                <HiaceCard key={hiace.id} hiace={hiace} />
              ))}
            </div>
          ) : (
            <motion.div
...
export default function HiaceListPage() {
  return (
