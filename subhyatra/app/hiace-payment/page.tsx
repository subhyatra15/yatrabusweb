...
function HiacePaymentPageComp() {
  ...
  useEffect(() => {
    if (id) {
      fetchHiaceBookingDetails(id);
    }
  }, [id]);

  const fetchHiaceBookingDetails = async (id: string) => {
    try {
      setIsLoading(true);
      const token = localStorage.getItem("accessToken");

      const response = await axios.get(`${API_URL}/api/v1/hiace-bookings/${id}`, {
        headers: {
      ...
      if (response.data) {
        ...
        const transformedData: BookingData = {
          vehicleName:
            data.schedule_details?.hiace_name ||
            data.schedule_details?.route_details?.hiace_name ||
            "Hiace",
          vehicleType: "Hiace",
          from: data.from_city || data.boarding_stop?.city || "Source",
          to: data.to_city || data.dropping_stop?.city || "Destination",
          date: formatDate(data.schedule_details?.departure_datetime),
          departure: formatTime(data.schedule_details?.departure_datetime),
          arrival: formatTime(data.schedule_details?.arrival_datetime),
          seats: data.booking_seats?.map((seat: any) => seat.seat_number?.toString()) || [],
          seatCount: data.booking_seats?.length || 0,
          pricePerSeat:
            data.booking_seats?.length > 0
              ? parseFloat(data.booking_seats[0].price) || 0
              : 0,
  ...
  const payWithEsewa = async (booking_id: number) => {
    try {
      ...
      const newWindow = window.open("", "eSewa Payment", "width=600,height=700,scrollbars=yes");
  ...
  const payWithKhalti = async (booking_id: number) => {
    try {
      ...
      const response = await axios.post(
        `${API_URL}/api/v1/payments/hiace/khalti/initiate/`,
        {
          booking_id: booking_id,
        },
        {
      ...
      const newWindow = window.open(d.payment_url, "Khalti Payment", "width=600,height=700,scrollbars=yes");
  ...
  const renderTimer = () => {
    ...
    return (
      <div className="bg-white mx-4 mt-4 rounded-xl border border-emerald-100/50 shadow-sm px-4 py-3">
        ...
        <div className="w-full h-1 bg-slate-200 rounded-full mt-2 overflow-hidden">
          <div
            className={cn(
              ...
              totalSeconds < 60 ? "bg-red-500" : "bg-emerald-600"
            )}
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-emerald-50/30">
        ...
        >
          ...
          <p className="mt-6 text-emerald-600 font-medium">Loading booking details...</p>
        </motion.div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50/20">
        ...
        <div className="max-w-6xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-lg font-bold text-gray-900">Hiace Payment</h1>
            </div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-4 pb-32">
              ...
              <motion.button
                ...
                className={cn(
                  ...
                  selectedPayment === method.id
                    ...
                    : "border-slate-200/50 hover:border-emerald-200",
        ...
        {isBookingValid && (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              />
              <button className="bg-emerald-50 text-emerald-600 font-semibold px-4 py-2 rounded-xl hover:bg-emerald-100 transition-colors">
                Apply
              </button>
            </div>
          </motion.div>
        ...
        <motion.div
          ...
          className="flex items-start gap-3 bg-emerald-50/30 p-4 rounded-xl border border-emerald-100/50"
        >
        ...
        >
          <div className="max-w-6xl mx-auto flex items-center justify-between">
            ...
            <motion.button
              ...
              className={cn(
                "bg-gradient-to-r from-emerald-600 to-emerald-500 text-white px-8 py-3.5 rounded-xl font-semibold flex items-center gap-2 shadow-lg shadow-emerald-500/25 hover:shadow-emerald-500/40 transition-all",
            ...
            >
              <div className="text-center">
                ...
                <p className="text-sm text-slate-500 mt-2">
                  Your Hiace booking has been confirmed. You will receive a confirmation email shortly.
                </p>
                ...
                <div className="grid grid-cols-2 gap-4 bg-slate-50 rounded-xl p-4 mt-4">
                  ...
                  <div className="text-center">
                    ...
                    <p className="font-bold text-emerald-600">Rs. {bookingData.grandTotal.toFixed(2)}</p>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full bg-linear-to-r from-emerald-600 to-emerald-500 text-white rounded-xl py-3.5 font-semibold mt-4 shadow-lg shadow-emerald-500/25 hover:shadow-emerald-500/40 transition-all"
                >
                  View My Hiace Bookings
                </button>
                <button
                  onClick={() => {
                    setShowSuccessModal(false);
...
export default function HiacePaymentPage() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <HiacePaymentPageComp />
    </Suspense>
  );
}