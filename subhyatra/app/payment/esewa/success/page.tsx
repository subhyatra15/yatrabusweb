...
function EsewaPaymentComp() {
  ...
  const verifyPayment = async (data: string) => {
    try {
      ...
      if (!uuid) {
        console.error("❌ No transaction UUID found:", paymentData);
        setStatus("error");
      ...
      console.log("📤 Sending verification for UUID:", uuid);
      ...
      // Check if payment was successful - handle different response structures
      const isSuccess = 
        ...
        response.data.status === "success" ||
        // Check for payment_status field
        response.data.payment_status === "COMPLETE" ||
        response.data.payment_status === "SUCCESS" ||
        response.data.payment_status === "success" ||
        // Check for message
        response.data.message?.toLowerCase().includes("success") ||
        response.data.message?.toLowerCase().includes("complete") ||
      ...
      if (!error.response) {
        ...
        if (errorData && (
          ...
          // The error response actually indicates success
          console.log("✅ Payment verification successful (from error response)!");
  ...
  const handleMessage = (event: MessageEvent) => {
    ...
    console.log("📨 Iframe message received:", url);

    if (typeof url !== "string") return;

    if (url.includes("/payment/esewa/success")) {
      try {
        ...
        if (data) {
          ...
          console.error("❌ No data parameter in callback URL");
          setStatus("error");
    ...
    if (url.includes("/payment/esewa/failure")) {
      setStatus("error");
      setErrorMessage("Your eSewa payment was not completed. Please try again.");
      return;
    }
  };
  ...
  // Check if we have a data parameter in the URL (for direct callback)
  useEffect(() => {
    if (dataParam && !verificationAttempted) {
      console.log("📨 Direct data parameter found in URL");
      setIsLoading(false);
      verifyPayment(dataParam);
    } else if (!html && !dataParam) {
      setIsLoading(false);
      setStatus("error");
      setErrorMessage("No payment data found. Please try again.");
    }
  }, [dataParam, html, verificationAttempted]);
        ...
        >
          ...
          {verificationData && (
            <div className="mt-4 text-xs text-slate-400">
              <p>Transaction ID: {verificationData.transaction_uuid || verificationData.id || 'N/A'}</p>
            </div>
          )}
        </motion.div>
      </div>
  ...
  // If we have data parameter but no html, show verification in progress
  if (dataParam && !html) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-slate-50 to-indigo-50/30">
        ...
        >
          ...
          <p className="mt-6 text-indigo-600 font-medium">Verifying your payment...</p>
          {isVerifying && (
  ...
  // Loading state for initial payment page
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-slate-50 to-indigo-50/30">
        <motion.div
  ...
  // Show the eSewa payment iframe (only when html is present)
  return (
      ...
      <main className="max-w-2xl mx-auto px-4 py-4">
        ...
        <div className="mt-4 text-center">
          <p className="text-xs text-slate-400">
            ...
            <a href="mailto:support@yatrabus.com" className="text-indigo-600 font-medium hover:underline">
